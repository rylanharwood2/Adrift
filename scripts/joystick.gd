...
func _input(event):
	...
	if event is InputEventScreenTouch or event is InputEventMouseButton or event is InputEventScreenDrag or (event is InputEventMouseMotion and dragging):
		...
		if (dragging and "pressed" not in event) or (event.pressed and touch_pos.distance_to(self.position) < base_radius):
			if touch_pos.distance_to(self.position) < base_radius:
				dragging = true
		...
		elif not event.pressed and not touch_buttons.shoot_pressed and not touch_buttons.boost_pressed:
			var ev := InputEventAction.new()
			ev.action = "move_left"
			ev.pressed = false
			Input.parse_input_event(ev)
			...
			touch_buttons.shoot_pressed = false
			touch_buttons.boost_pressed = false
